...
import { UniversalSigning } from '../utilities/UniversalSigning';

const MainNetCoinJSON = require('../contracts/THXTokenRinkeby.json');
const LoomCoinJSON = require('../contracts/THXToken.json');
...
export class Network extends UniversalSigning {
    ...
    web3Ethereum: any;
    web3Loom: any;
    mainNetGatewayContract: any;
    loomGatewayContract: Contracts.TransferGateway;
    mainNetCoinContract: any;
    loomCoinContract: any;
    rinkebyNetworkConfig: any;
    extdevNetworkConfig: any;
    ethereumGatewayContract: any;
    ...
    async _getContracts(client: any, web3Ethereum: any, web3Loom: any, accountMapping: any) {
    ...
    async _updateBalances() {
        ...
        this._events.dispatch('updateBalances', { mainNetBalance: mainNetBalance, loomBalance: loomBalance })
    }
    ...
    async withdrawERC20(amount: number) {
    ...
    async _transferCoinsToLoomGateway(amount: number) {
        const amountInWei = this.web3Ethereum.utils.toWei(amount.toString(), 'ether')
    ...
    async _withdrawCoinsFromMainNetGateway(receipt: any) {
    ...
    async _filterEvents() {
        this.loomCoinContract.events.Transfer({ filter: { address: this.accountMapping.plasma.local.toString() } }, async (err: string, event: any) => {
            if (err) console.error('Error on event', err)
            await this._updateBalances()
        })
        this.mainNetCoinContract.events.Transfer({ filter: { address: this.accountMapping.ethereum.local.toString() } }, async (err: string, event: any) => {
            if (err) console.error('Error on event', err)
            await this._updateBalances()
        })
    }
